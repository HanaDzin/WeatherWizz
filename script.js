...
//icon changes based on weather codes
const weatherCodes = {
  clear: [1000],
...
const displayHourlyForecast = (hourlyData) => {
  ...
  //filtering hourly data to only get the next 24 hours
  ...
  //generating HTML for the hourly forecast & displaying it
...
const getWeatherDetails = async (API_URL) => {
  ...
  try {
    ...
    //extracting needed data (temp, desc & icon)
    const temperature = Math.floor(data.current.temp_c);
    const description = data.current.condition.text;
    const weatherIcon = Object.keys(weatherCodes).find((icon) => weatherCodes[icon].includes(data.current.condition.code));

    //updating the components to show this data
